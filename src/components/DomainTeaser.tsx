...
export default function DomainTeaser() {
  ...
  return (
    <section className="w-full mb-32 relative overflow-hidden">
      {/* Soft ambient glow */}
      ...
      {/* ── Section header ── */}
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-6 border-b border-black/5 pb-8">
        <div>
          ...
          <p className="text-xs text-muted-steel mt-2 max-w-md leading-relaxed font-light">
        ...
        <motion.a
          ...
          className="flex items-center gap-2 px-6 py-3 bg-cosmic-teal text-white rounded-2xl text-xs font-mono font-bold uppercase tracking-wider shadow-sm hover:bg-[#D94F2E] transition-colors duration-350 cursor-pointer shrink-0"
        >
          Browse All Domains
      ...
      <form onSubmit={handleSearch} className="mb-10">
        <div className="relative max-w-xl">
          ...
          <input
            ...
            className="w-full bg-abyss-panel/60 border border-black/8 text-star-white placeholder:text-muted-steel/50 text-sm font-sans py-3.5 pl-11 pr-36 rounded-2xl focus:outline-none focus:border-cosmic-teal/40 transition-all duration-350"
          />
          <button
            type="submit"
            className="absolute right-2 top-1/2 -translate-y-1/2 px-5 py-2 bg-cosmic-teal text-white rounded-xl text-[10px] font-mono font-bold uppercase tracking-wider transition-colors duration-350 hover:bg-[#D94F2E] cursor-pointer"
          >
            Search
          </button>
        </div>
      ...
      >
        {spotlightDomains.map((domain) => (
          <motion.a
            ...
            className="group flex items-center justify-between bg-abyss-panel/50 border border-black/6 rounded-2xl px-5 py-4 hover:border-[#E85D3B]/20 hover:shadow-md hover:shadow-[#E85D3B]/5 transition-all duration-400 cursor-pointer"
          >
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <div className="flex items-baseline gap-0.5">
                  <span className="font-display font-black text-base text-star-white tracking-tight group-hover:text-cosmic-teal transition-colors duration-400 truncate">
                    {domain.name}
                  </span>
