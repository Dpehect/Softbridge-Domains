...
export function LivePreviewViewport() {
  ...
  return (
    <div className="flex flex-col items-center gap-3 w-full h-full">
      ...
      <div className="flex items-center gap-2 bg-white/5 border border-white/5 px-3 py-1.5 rounded-xl backdrop-blur-md">
        <button
      ...
      {/* Frame Container */}
      <div className="w-full flex-1 flex items-center justify-center p-3 bg-deep-void/45 rounded-2xl border border-white/5 overflow-hidden relative">
        <div className="absolute inset-0 grid-bg opacity-40 pointer-events-none" />

        {/* Viewport Frame */}
        <motion.div
          layout
          className={`glass-panel rounded-xl overflow-y-auto overflow-x-hidden shadow-[0_20px_50px_rgba(92,46,31,0.14)] border border-white/10 transition-all duration-300 custom-theme-preview flex flex-col relative ${widthClasses[previewDevice]}`}
          style={themeStyles}
        >
          {/* Mock Browser Header */}
          <div className="bg-black/25 px-3 py-1.5 border-b border-white/5 flex items-center gap-2 sticky top-0 z-30 backdrop-blur-md">
          ...
          {config.templateId ? (
            <div className="flex-1 flex flex-col">
              {/* Preview Navigation */}
              <nav className="px-5 py-3 flex items-center justify-between border-b border-white/5 bg-black/10 backdrop-blur-sm">
    ...
    case "hero":
      return (
        <section className="px-5 py-12 text-center border-b border-white/5 relative overflow-hidden flex flex-col items-center justify-center min-h-[250px]">
          ...
          <p className="text-xs md:text-sm text-nebula-slate max-w-md mb-5 leading-relaxed">
            {content.subheading}
          </p>
          <button
            style={{
    ...
    case "contact":
      return (
        <section className="px-5 py-9 border-b border-white/5 text-center">
          ...
          <p className="text-xs text-nebula-slate mb-5">{content.description}</p>
